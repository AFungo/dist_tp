...
from networking.ticket_service.ticket_service_pb2 import AbortReply, AbortRequest, BuyFlightPackageReply, CommitReply, CommitRequest, FlightsByRouteReply, VoteReply, VoteRequest, TSAllFlightsReply
...
class TicketServiceServicer(TicketServiceServicer):
    ...
    async def GetAllFlights(self, request, context):
        ...
        return TSAllFlightsReply(all_flights=json.dumps(flights))
    ...
    async def BuyFlightPackage(self, request, context):
        ...
        if not all(False if not reserve else reserve.is_temp_reserved for reserve in reserve_results):
            flights_to_cancel = [
                request.flight_id for request in reserve_results if request != False and request.is_temp_reserved 
            ]
    ...
    async def _try_operation(self, operation, neighbor, request, operation_name):
        try:
    ...
    async def _collect_votes(self, flights_id, seats_amount):
        ...
        return [response.vote for response in responses if response is not None]
    ...
    def _can_reserve(self, neighbor_id, flights_id, seats_amount, seats_available, timestamp):
        pending_logs = [
            log for log in self.logs if log.is_pending() and self._intersection(log.flights_id, flights_id) is not []
        ]
    ...
    async def _get_seats_available(self, flights_id):
        seats_available = {}
        for f in flights_id:
            stub = self.airline_flights.get(f)
            if not stub:
                raise Exception("Airline not found.")
